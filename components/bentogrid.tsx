...
const BentoCard = ({
  ...
  <motion.div
    ...
    className={`relative overflow-hidden border border-border bg-card z-10 ${className}`}
  >
    {children}
  </motion.div>
);
...
export const BentoGrid = () => {
  ...
  const getCalendarDays = (monthIndex: number, year: number = 2025) => {
    ...
    for (let i = 0; i < firstDay; i++) {
      days.push({ num: daysInPrevMonth - firstDay + i + 1, current: false, prev: true });
    }
    ...
    <section className="w-full relative bg-background py-10 md:py-20">
      ...
      <div className="absolute inset-0 w-[90%] md:w-[85%] mx-auto pointer-events-none">
        <div className="absolute inset-0 grid grid-cols-5 h-full opacity-100 dark:opacity-50">
          {/* Column 1 */}
          <div className="relative col-start-2 row-span-full border-x border-primary/10 bg-[size:10px_10px] bg-fixed"
            style={{
              backgroundImage: 'repeating-linear-gradient(315deg, oklch(var(--primary)/0.2) 0, oklch(var(--primary)/0.1) 1px, transparent 0, transparent 50%)'
            }}
          ></div>
          {/* Column 3 */}
          <div className="relative col-start-4 row-span-full border-x border-primary/10 bg-[size:10px_10px] bg-fixed"
            style={{
              backgroundImage: 'repeating-linear-gradient(315deg, oklch(var(--primary)/0.2) 0, oklch(var(--primary)/0.1) 1px, transparent 0, transparent 50%)'
            }}
          ></div>
      ...
      <div className="container w-[90%] md:w-[80%] lg:w-[75%] mx-auto relative z-20">
        <div className="mb-10 md:mb-16 max-w-2xl">
          <h2 className="text-3xl md:text-4xl font-bold mb-4 text-foreground font-display uppercase tracking-tight">
            Features for<br />Modern Creators
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-6 md:grid-rows-4 gap-4 h-auto md:h-[850px]">
          ...
          <BentoCard className="md:col-span-4 md:row-span-2 flex flex-col p-5 md:p-8 hover:border-primary/30 transition-colors">
            ...
            <div className="absolute top-4 right-4 md:top-6 md:right-6">
              <div className="flex items-center gap-2 px-2 py-1 bg-background/80 backdrop-blur border border-border rounded-full">
                <span className="relative flex h-2 w-2">
                  <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-success opacity-75"></span>
                  <span className="relative inline-flex rounded-full h-2 w-2 bg-success"></span>
                </span>
                <span className="text-[10px] font-mono text-success uppercase tracking-wider">System Online</span>
              </div>
            </div>

            <div className="relative z-10 mb-6">
              <div className="flex items-center gap-3 mb-4">
                <div className="p-2 border border-border bg-muted text-foreground">
                  <LayoutTemplate size={20} />
                </div>
                <h3 className="text-lg md:text-xl font-bold text-foreground font-display uppercase">Smart Editor</h3>
              </div>
            ...
            <div className="flex-1 relative bg-muted/20 border border-border overflow-hidden flex items-center justify-center min-h-[200px]">
              {/* Editor UI Shell */}
              <div className="absolute inset-4 border border-border bg-card shadow-2xl flex flex-col">
                {/* Header */}
                <div className="h-8 border-b border-border flex items-center px-3 justify-between bg-muted/30">
                  ...
                  <div className="text-[10px] text-muted-foreground uppercase tracking-widest">Editor_v2.0</div>
                </div>
                  ...
                  <AnimatePresence mode="wait">
                    {editorStep === 0 && (
                      <motion.div
                        ...
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                        className="relative w-3/4 h-3/4 bg-white p-6 shadow-lg flex flex-col gap-3 items-center justify-center border border-border"
                      >
                        <motion.h4
                    ...
                    {editorStep === 2 && (
                      ...
                      >
                        ...
                        <div className="relative z-10 flex flex-col items-center">
                          ...
                          <div className="text-xs text-white animate-pulse font-display uppercase tracking-widest">Magic Effects</div>
                        </div>
                      </motion.div>
                    )}
                  </AnimatePresence>
                ...
                <div className="h-10 border-t border-border flex items-center justify-around text-muted-foreground bg-muted/30">
                  <div className={`flex items-center gap-1 transition-colors ${editorStep === 0 ? 'text-primary' : ''}`}>
                    <Edit3 size={10} />
                    <span className="text-[8px] uppercase hidden sm:inline">Typography</span>
                  </div>
                  <div className={`flex items-center gap-1 transition-colors ${editorStep === 1 ? 'text-primary' : ''}`}>
                    <Move size={10} />
                    <span className="text-[8px] uppercase hidden sm:inline">Structure</span>
                  </div>
                  <div className={`flex items-center gap-1 transition-colors ${editorStep === 2 ? 'text-primary' : ''}`}>
                    <Sparkles size={10} />
                    <span className="text-[8px] uppercase hidden sm:inline">Effects</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          >
            <div className="w-full flex flex-col items-center">
              <div className="p-3 border border-border bg-muted text-foreground mb-6">
                <Type size={24} />
              </div>
              <h3 className="text-lg font-bold mb-2 font-display uppercase text-foreground">Typography</h3>
            ...
            {/* Main Display */}
            <div className="relative w-full flex-1 flex items-center justify-center bg-muted/20 border border-border mb-4 overflow-hidden min-h-[120px]">
              <motion.div
                key={fontIndex}
            ...
            {/* Toolbar */}
            <div className="w-full flex items-center justify-between p-2 border border-border bg-muted/30">
              ...
              <div className="flex gap-1">
                <button onClick={() => toggleStyle('bold')} className={`p-1.5 rounded transition-colors ${activeStyles.includes('bold') ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:bg-muted'}`}><Bold size={12} /></button>
                <button onClick={() => toggleStyle('italic')} className={`p-1.5 rounded transition-colors ${activeStyles.includes('italic') ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:bg-muted'}`}><Italic size={12} /></button>
              </div>
            </div>
          </BentoCard>
          ...
          >
            <div className="flex justify-between items-start mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-bold font-display uppercase text-foreground">Schedule</h3>
                  <p className="text-xs text-muted-foreground">Auto-post magic.</p>
                </div>
              </div>

              <div className="flex items-center border border-border bg-muted/30">
                <button onClick={() => handleMonthChange('down')} className="p-2 hover:text-foreground text-muted-foreground transition-colors"><ChevronDown size={14} className="rotate-90" /></button>
                <div className="px-3 py-1 text-xs font-mono text-foreground border-x border-border min-w-[70px] md:min-w-[80px] text-center">{months[currentMonth].substring(0, 3)}</div>
                <button onClick={() => handleMonthChange('up')} className="p-2 hover:text-foreground text-muted-foreground transition-colors"><ChevronUp size={14} className="rotate-90" /></button>
              </div>
            </div>

            {/* Calendar Visual */}
            <div className="flex-1 border border-border bg-muted/10 p-2 md:p-4 relative overflow-hidden flex flex-col min-h-[200px]">
              ...
              <div className="relative z-10 grid grid-cols-7 gap-1 h-full content-stretch">
                {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((d, i) => (
                  <div key={i} className="text-[10px] text-muted-foreground text-center py-1 font-mono">{d}</div>
                ))}

                {calendarDays.map((day, i) => {
                  // Simple deterministic logic for demo
                  const isScheduled = day.current && (day.num * (currentMonth + 2)) % 5 === 0;
                  ...
                  return (
                    <div
                      ...
                      className={`
                        ...
                        `}
                    >
                      <span className={`text-[10px] ${isScheduled ? 'font-bold text-foreground' : ''}`}>{day.num}</span>
          ...
          >
            <div className="flex justify-between items-start mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-bold font-display uppercase text-foreground">Live Preview</h3>
                  <p className="text-xs text-muted-foreground">Pixel perfect.</p>
                </div>
              </div>

              <div className="flex border border-border bg-muted/30 p-0.5">
                <button
                  onClick={() => setPreviewMode('mobile')}
                  className={`p-2 transition-all duration-300 ${previewMode === 'mobile' ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}
                >
                  <Smartphone size={14} />
                </button>
                <button
                  onClick={() => setPreviewMode('desktop')}
                  className={`p-2 transition-all duration-300 ${previewMode === 'desktop' ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}
                >
                  <Monitor size={14} />
                </button>
              </div>
            ...
            {/* Preview Area */}
            <div className="flex-1 bg-muted/20 border border-border flex items-center justify-center relative overflow-hidden py-6 perspective-container min-h-[250px]">
              <TiltCard className="w-full h-full flex items-center justify-center">
                <AnimatePresence mode="wait">
                  <motion.div
                    ...
                    className="bg-background border border-border shadow-2xl flex flex-col overflow-hidden relative group/device mx-auto"
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="w-full aspect-video bg-muted/30 rounded border border-border flex items-center justify-center overflow-hidden">
                      ...
                      {/* Post 3 */}
                      <div className="w-full aspect-square bg-muted/30 rounded border border-border mt-2"></div>
                    </motion.div>
