...
export const Pricing: React.FC = () => {
  ...
  return (
    <section className="w-full py-20 relative">
      <div className="container w-[90%] md:w-[70%] lg:w-[60%] mx-auto">
        
        <div className="flex flex-col items-center mb-20">
            <div className="relative mb-8">
                <div className="border border-border px-8 py-4 md:px-12 md:py-6 bg-background/50 backdrop-blur-sm relative z-10">
                ...
                <EtchingCorner className="top-[-1px] left-[-1px] border-t border-l" />
                <EtchingCorner className="bottom-[-1px] right-[-1px] rotate-180 border-t border-l" />
            </div>
            ...
            <div className="flex flex-col md:flex-row items-center gap-6">
                ...
                {/* Toggle */}
                <div className="flex items-center gap-1 bg-muted/50 border border-border p-1">
                    <button 
                        onClick={() => setIsAnnual(false)}
                        className={`px-4 py-2 text-xs font-bold uppercase tracking-wider transition-colors ${!isAnnual ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}
                    >
                        Monthly
                    </button>
                    <button 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {/* Free Tier */}
            <SpotlightCard 
                className="border border-border bg-card p-6 md:p-12 flex flex-col hover:border-foreground/30 transition-colors"
            >
                <div className="mb-8 relative z-10">
                    <div className="text-muted-foreground text-xs font-bold uppercase tracking-widest mb-2">Starter</div>
                    <div className="text-4xl font-display font-bold text-foreground">$0<span className="text-lg text-muted-foreground font-normal">/mo</span></div>
                </div>

                <ul className="space-y-4 mb-12 flex-1 relative z-10">
                    {['3 Projects', 'Basic Templates', '720p Exports', 'Community Support'].map((item, i) => (
                        <li key={i} className="flex items-center gap-3 text-sm text-muted-foreground">
                            <Check size={16} className="text-succes" />
                            {item}
                        </li>
                    ))}
                     {['Team Collaboration', 'AI Magic Tools', 'Custom Fonts'].map((item, i) => (
                        <li key={i} className="flex items-center gap-3 text-sm text-muted-foreground/50 line-through decoration-border">
            ...
            {/* Premium Tier */}
            <SpotlightCard 
                className="border border-primary/20 bg-gradient-to-b from-card to-muted/20 p-6 md:p-12 flex flex-col hover:border-primary/50 transition-colors duration-500"
            >
                ...
                <div className="mb-8 relative z-10">
                    <div className="flex justify-between items-start">
                        <div className="text-foreground text-xs font-bold uppercase tracking-widest mb-2 flex items-center gap-2">
                            Pro <span className="px-2 py-0.5 bg-foreground text-background text-[10px]">Popular</span>
                        </div>
                    </div>
                ...
                <ul className="space-y-4 mb-12 flex-1 relative z-10">
                    {['Unlimited Projects', 'Premium Templates', '4K Exports', 'Priority Support', 'Team Collaboration', 'AI Magic Tools', 'Custom Fonts'].map((item, i) => (
